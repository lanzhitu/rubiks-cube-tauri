...
export const SOLVING_STAGES: Stage[] = [
    {
        ...
        hints: [
            '目标：做出白色十字，并使每个白色棱块的侧色与对应中心对齐',
            '一次只处理一个白色棱块，尽量不要破坏已放好的棱',
            '顶层但翻转错误：F U\' R U（翻正该棱）',
            '前层且直接上去会翻错：先 F2，再 F U\' R U（等价 F\' U\' R U）',
            '中层：用 U\' R U 或 U L\' U\' 把它顶到顶层，再按侧色对齐插入',
            '底层：用 D / D\' 把棱转到目标位置正下方，然后按上面方法插入',
            '每放好一条白棱都检查侧色是否与中心对齐，直到完成 4 条'
        ],
